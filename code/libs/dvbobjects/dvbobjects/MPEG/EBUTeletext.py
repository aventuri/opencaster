...
# return pack("!BBBBBBBBBBBBBB",
            return pack("BBBBBBBBBBBBBB",
                        ...
                        0x2C,  # 44 bytes
                        0x01 << 6 | (self.field_parity &
                                     0x01) << 5 | self.line_offset & 0x1F,
...
# fmt = "!BBBBBBB%ds%ds%ds%ds%ds%dsBH" % (len(link1bytes), len(link2bytes), len(link3bytes), len(link4bytes), len(link5bytes), len(link6bytes))
            fmt = "BBBBBBB%ds%ds%ds%ds%ds%dsBH" % (len(link1bytes), len(link2bytes), len(
                link3bytes), len(link4bytes), len(link5bytes), len(link6bytes))
            return pack(fmt,
                        self.data_unit_id,  # 0x02 non-subtile or 0x03 for subtitles
                        0x2C,  # 44 bytes
                        0x01 << 6 | (self.field_parity &
                        ...
                        hamming1,
                        hamming2,
                        # designation code allows 2^4 = 16 different pages more
                        invert(hamming84(self.designation_code)),
                        link1bytes,
                        link2bytes,
                        link3bytes,
                        link4bytes,
