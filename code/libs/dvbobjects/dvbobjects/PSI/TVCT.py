...
class channel_loop_item(DVBobject):
    ...
    # pack event_descriptor_loop
        ...
        return pack(fmt,
                    self.short_name,
                    0xF0000000 | ((self.major_channel_number & 0x3FF) << 18) | (
                        (self.minor_channel_number & 0x3FF) << 8) | (self.modulation_mode & 0xFF),
                    self.carrier_frequency,
                    self.channel_TSID,
                    self.program_number,
                    (self.ETM_location << 6) | (self.access_controlled << 5) | (
                        self.hidden << 4) | 0xC | (self.hide_guide << 1) | 0x1,
