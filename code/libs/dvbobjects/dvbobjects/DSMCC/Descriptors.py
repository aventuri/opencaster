...
class stream_event_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(
            FMT,
            self.event_id,
            # 31 bits reserved, 1 bit NPT_reference
            self.reserved | (self.eventNPT >> 32),
...
class stream_event_ginga_descriptor(stream_event_descriptor):
    ...
    def bytes(self):
        ...
        self.private_data = pack(FMT,
                                 3+len(self.payload),
                                 self.command_tag,
                                 ((self.sequence_number & 0x7F) << 1) | (
                                     self.final_flag & 0x01),
                                 self.payload,
                                 self.FCS
                                 )
...
class npt_reference_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(
            FMT,
            # 1 bit postcontdesc, 7 bits contentId
            (self.post_continuity_indicator << 7) | (self.content_id & 0x7F),
            # 31 bits reserved, 1 bit STC_reference
            0xFE | (self.STC_reference >> 32),
            self.STC_reference & 0xFFFFFFFF,     # 32 bits STC_reference
            # 31 bits reserved, 1 bit NPT_reference
            0xFFFFFFFE | (self.NPT_reference >> 32),
...
class npt_endpoint_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(
            FMT,
            # 15 bits reserved, 1 bit NPT_start
            0xFFFE | (self.NPT_start >> 32),
            self.NPT_start & 0xFFFFFFFF,      # 32 bits STC_reference
            # 31 bits reserved, 1 bit NPT_stop
            0xFFFFFFFE | (self.NPT_stop >> 32),
