...
class software_download_trigger_table(Section):
    ...
    def pack_section_body(self):
        ...
        return pack(fmt,
                    ...
                    self.service_id,
                    # number of contents, should be length in bytes instead of qty.??
                    len(self.contents),
                    content_bytes)
...
class sdtt_content_loop_item(DVBobject):
    ...
    def pack(self):
        ...
        return pack(fmt,
                    ...
                    (self.new_version << 4) | (
                        self.download_level << 2) | self.version_indicator,
                    ((len(schedule_loop_bytes)+len(descriptor_loop_bytes)) << 4) | 0x0F,
                    ((len(schedule_loop_bytes)) <<
...
class sdtt_schedule_loop_item(DVBobject):
    def pack(self):
        fmt = ("!HBBBBBB")
        return pack(fmt,
                    int(MJD_convert(self.start_year,
...
class download_content_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(
            ...
            # sum of data sizes transmitted by the carousel in bytes.
            self.component_size,
            self.download_id,     # same as the one transmitted in DII/DDB
            # timeout value in ms for all DII sections of the carousel.
            self.time_out_value_DII,
            # leak_rate. # TODO CHECK its ok...
            self.leak_rate >> 14, ((self.leak_rate << 2) | 0x3) & 0xFFFF,
            self.component_tag,
            len(self.compatibility_descriptor_bytes),
